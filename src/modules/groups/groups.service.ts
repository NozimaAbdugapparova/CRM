...
export class GroupsService {
  ...
  async getOneGroup(groupdId: number) {
    const existGroup = await this.prisma.group.findFirst({
      where: { id: groupdId, status: Status.active },
      select: {
        id: true, name: true, max_student: true,
        start_date: true, start_time: true, week_day: true, created_at: true,
        courses:  { select: { id: true, name: true, duration_month: true } },
        rooms:    { select: { id: true, name: true } },
        teachers: { select: { id: true, first_name: true, last_name: true, photo: true } },
      },
    });
    if (!existGroup) throw new NotFoundException('group is not found with this id');
    return { success: true, data: existGroup };
  }
  ...
  async getOneGroupStudents(groupdId: number) {
    ...
    const groupStudents = this.prisma.studentGroup.findMany({
      where: { group_id: groupdId, status: Status.active },
      select: {
        students: {
          select: {
            id: true, first_name: true, last_name: true,
            phone: true, email: true, photo: true,
            birth_date: true, created_at: true,
          },
        },
      },
    });
    ...
    return { success: true, data: dataFormatter };
  }

  async getAllGroups(search: FilterDto) {
    const { groupName, maxStudent } = search;
    const where: any = { status: Status.active };
    if (groupName)   where['name']        = groupName;
    if (maxStudent)  where['max_student'] = +maxStudent;

    const groups = await this.prisma.group.findMany({
      where,
      select: {
        id: true, name: true, max_student: true,
        start_date: true, start_time: true, week_day: true,
  ...
  async createGroup(payload: CreateGroupDto) {
    ...
    if (!existCourse) throw new NotFoundException('Course is not found by this ID');

    const existTeacher = await this.prisma.teacher.findFirst({
      where: { id: payload.teacher_id, status: Status.active },
    });
    if (!existTeacher) throw new NotFoundException('Teacher is not found by this ID');

    const existGroup = await this.prisma.group.findUnique({
      where: { name: payload.name },
    });
    if (existGroup) throw new ConflictException('This group is already exist');

    const existRoom = await this.prisma.room.findFirst({
      where: { id: payload.room_id, status: Status.active },
    });
    if (!existRoom) throw new NotFoundException('Room is not found by this ID');

    const existRoomGroups = await this.prisma.group.findMany({
      where: { room_id: payload.room_id, status: Status.active },
      select: { start_time: true, courses: { select: { duration_hours: true } } },
    });
    ...
    await this.prisma.group.create({
      data: { ...payload, start_date: new Date(payload.start_date) },
    });
    return { success: true, message: 'Group is created' };
  }
  ...
  async updateGroup(id: number, payload: UpdateGroupDto) {
    ...
    if (!existing) throw new NotFoundException('Group is not found with this id');

    // Nom o'zgarayotgan bo'lsa duplicate tekshirish
    if (payload.name && payload.name !== existing.name) {
      const nameTaken = await this.prisma.group.findUnique({
        where: { name: payload.name },
      });
      if (nameTaken) throw new ConflictException('This group name is already taken');
    }
    ...
    const updated = await this.prisma.group.update({
      ...
      data: {
        ...
        ...(payload.max_student && { max_student: payload.max_student }),
        ...(payload.start_time  && { start_time:  payload.start_time }),
        ...(payload.week_day    && { week_day:     payload.week_day as any }),
        ...(payload.start_date  && { start_date:   new Date(payload.start_date) }),
        ...(payload.course_id   && { courses:   { connect: { id: payload.course_id   } } }),
        ...(payload.teacher_id  && { teachers:  { connect: { id: payload.teacher_id  } } }),
        ...(payload.room_id     && { rooms:     { connect: { id: payload.room_id     } } }),
      },
    });
